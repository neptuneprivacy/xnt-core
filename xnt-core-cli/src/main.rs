...
use neptune_privacy::application::rpc::server::coinbase_output_readable::CoinbaseOutputReadable;
...
use neptune_privacy::state::wallet::utxo_notification::PrivateNotificationData;
use neptune_privacy::state::wallet::utxo_notification::UtxoNotificationMedium;
use neptune_privacy::state::wallet::wallet_file::WalletFile;
...
enum Command {
    ...
    /******** READ STATE ********/
    /// retrieve network that xnt-core is running on
    Network,

    /// retrieve address for peers to contact this xnt-core node
    OwnListenAddressForPeers,

    /// retrieve instance-id of this xnt-core node
    OwnInstanceId,
...
struct Config {
    /// Sets the xnt-core rpc server localhost port to connect to.
    #[clap(short, long, default_value = "9899", value_name = "port")]
    port: u16,

    /// xnt-core data directory containing wallet and blockchain state
    #[clap(long)]
...
async fn main() -> Result<()> {
    ...
    match &args.command {
        ...
        Command::GenerateWallet { network } => {
            ...
            println!(
                "Wallet stored in: {}\nMake sure you also see this path if you run the xnt-core client",
    ...
    let auth::CookieHint {
        ...
        Err(e) => {
            eprintln!("{e}");
            eprintln!(
                "Could not ping xnt-core. Do configurations match? Or is it still starting up?"
            );
            std::process::exit(1);
        }
    };
