...
pub const MAGIC_STRING_REQUEST: &[u8; 15] = b"8B8AB7FC438F411";
pub const MAGIC_STRING_RESPONSE: &[u8; 15] = b"Private money.\n";
...
pub const NEPTUNE_BANNER: &str = include_str!("assets/xnt-banner.txt");
...
pub async fn initialize(cli_args: cli_args::Args) -> Result<MainLoopHandler> {
    ...
    info!("Starting xnt-core node on {}.", cli_args.network);
    ...
    if global_state_lock.cli().rest_api_enable == true {
        if let Some(rest_port) = global_state_lock.cli().rest_port {
            if let Some(rest_listening_address) = &global_state_lock.cli().rest_listening_address {
                ...
                // each time we start xnt-core a new RPC cookie is generated.
                let valid_tokens: Vec<crate::application::rpc::auth::Token> = vec![
                ...
                let rpc_join_handle = tokio::spawn(async move {
                    ...
                    crate::application::rest_server::run_rpc_server(rest_listener, server)
                        .await
