...
impl TimeLockV2 {
    ...
    /// The `type_script_hash` that `TimeLockV2` coins carried *before* the
    /// `UpgradeVM` hard fork (Triton VM v3). `TimeLockV2` was introduced under
    /// triton-vm v2, where it hashed to this value; the v3 VM upgrade re-hashes
    /// every consensus program, changing it to the current `Self.hash()`.
    ///
    /// Coins committed to the mutator set between the +1-year timelock fork and
    /// the VM upgrade embed this legacy digest, so spending or collecting them
    /// post-fork requires remapping this value to the current `Self.hash()`.
    ...
    pub fn legacy_type_script_hash() -> Digest {
        Digest::try_from_hex(Self::LEGACY_TYPE_SCRIPT_HASH_HEX)
            .expect("legacy time-lock-v2 type-script-hash hex is valid")
    }
...
impl TimeLockV2 {
    ...
    pub(crate) fn library_and_code_for_targets(
        ...
        // V3 REMAP: programmatic `push` sequences for the timelock hashes the
        // program must recognize (see `push_digest_reversed`):
        //  - legacy + current TimeLock     -> extension-capable branch
        //  - current (self) + legacy TimeLockV2 -> plain branch
        let push_legacy_timelock = push_digest_reversed(TimeLock::legacy_type_script_hash());
        let push_current_timelock = push_digest_reversed(TimeLock.hash());
        let push_legacy_timelock_v2 = push_digest_reversed(TimeLockV2::legacy_type_script_hash());
        ...
        read_mem 5
        pop 1
        // V3 REMAP: extension-capable branch. Matches a coin tagged with EITHER
        // the legacy (pre-UpgradeVM) OR the current TimeLock hash. The +1 year
        // extension only applies to genesis target UTXOs (is_target gate), so
        // current-era TimeLock coins get a plain lock. `[tsh]` is preserved (via
        // dups) for the subsequent plain check.
        dup 4
        dup 4
        dup 4
        dup 4
        dup 4
        {&push_legacy_timelock}
        call tasmlib_hashing_eq_digest
        // _ ... [tsh] (tsh == legacy_timelock)
        dup 5
        dup 5
        dup 5
        dup 5
        dup 5
        {&push_current_timelock}
        call tasmlib_hashing_eq_digest
        // _ ... [tsh] is_legacy is_current
        add
        // _ ... [tsh] (tsh in {legacy, current} timelock)
        push 1
        swap 1
        skiz
        ...
        // V3 REMAP: plain branch. Matches a coin tagged with EITHER the current
        // TimeLockV2 (self) OR the legacy (pre-UpgradeVM) TimeLockV2 hash. This
        // is the last check, so it consumes `[tsh]` (swap 5; pop 5).
        dup 4
        dup 4
        dup 4
        dup 4
        ...
        pop 1
        call tasmlib_hashing_eq_digest
        // _ ... [tsh] (tsh == self_digest)
        dup 5
        dup 5
        dup 5
        dup 5
        dup 5
        {&push_legacy_timelock_v2}
        call tasmlib_hashing_eq_digest
        // _ ... [tsh] is_self is_legacy_v2
        add
        // _ ... [tsh] (tsh in {self, legacy} timelock_v2)
        swap 5
        pop 5
        // _ ... (tsh in {self, legacy} timelock_v2)
        push 1
        swap 1
        skiz
...
mod tests {
    ...
    fn time_lock_v2_source(target_utxo_digests: &[Digest]) {
        ...
        // V3 REMAP: the UpgradeVM fork re-hashed TimeLock/TimeLockV2. Recognise
        // the current TimeLock hash (what `TimeLock::until` now creates) in the
        // extension branch, and the legacy TimeLockV2 hash in the plain branch,
        // so timelocks committed across the fork stay lock-enforced. Compute the
        // current hash the same way the TASM does (`TimeLock.hash()`) so the two
        // can never drift if `TimeLock` is re-hashed again.
        let current_timelock_hash: Digest = TimeLock.hash();
        let legacy_timelock_v2_hash: Digest = TimeLockV2::legacy_type_script_hash();
        // DIGEST FILTER: a UTXO whose Tip5::hash(&Utxo) matches one of the
        // targets gets a +1 year extension on its TimeLock (legacy or current)
        // coin's release date. The extension applies only in the extension
        // branch below, never to the plain TimeLockV2 branch.
        ...
        while i < input_utxos.len() {
            ...
            while j < coins.len() {
                ...
                if coin.type_script_hash == old_timelock_hash
                    || coin.type_script_hash == current_timelock_hash
                {
                ...
                if coin.type_script_hash == self_digest
                    || coin.type_script_hash == legacy_timelock_v2_hash
                {
                    // extract state
                    let state: &Vec<BFieldElement> = &coin.state;
    ...
    fn make_witness_with_old_hash_coins(
        ...
        make_witness_with_tagged_coins(TimeLock.hash(), release_date, tx_timestamp)
    }

    /// Like [`make_witness_with_old_hash_coins`] but tags the rewritten timelock
    /// coins with an arbitrary `tag` hash, so the V3 remap's recognition of the
    /// pre-UpgradeVM TimeLock / TimeLockV2 hashes can be exercised directly.
    fn make_witness_with_tagged_coins(
        tag: Digest,
        release_date: Timestamp,
        tx_timestamp: Timestamp,
    ) -> TimeLockV2Witness {
                ...
                let rewritten_coins: Vec<Coin> = utxo
                    ...
                    .map(|coin| {
                        if coin.type_script_hash == new_v2_hash {
                            // Replace the V2 coin with a `tag`-hashed coin at the
                            // chosen release_date.
                            Coin {
                                type_script_hash: tag,
    ...
    #[test]
    fn legacy_tagged_timelock_coins_are_lock_enforced() {
        // V3 SECURITY: a coin committed before the UpgradeVM fork carries a
        // legacy TimeLock (4b4d2519…) or legacy TimeLockV2 (ac58ee89…) hash. The
        // collect remap routes such a coin's proof obligation to TimeLockV2, so
        // the TimeLockV2 program itself must still enforce the release date —
        // otherwise the remap would let a time-locked coin be spent early.
        //
        // The seeded UTXOs are NOT among the +1 year targets, so the extension
        // never applies and the plain lock holds: spendable iff release < now.
        for tag in [
            TimeLock::legacy_type_script_hash(),
            TimeLockV2::legacy_type_script_hash(),
        ] {
            // (a) still locked: release in the future -> rejected by rust AND tasm.
            let locked = make_witness_with_tagged_coins(
                tag,
                Timestamp::now() + Timestamp::days(1),
                Timestamp::now(),
            );
            assert!(
                TimeLockV2
                    .run_rust(&locked.standard_input(), locked.nondeterminism())
                    .is_err(),
                "legacy-tagged locked coin must be rejected (rust), tag {tag}"
            );
            assert!(
                TimeLockV2
                    .run_tasm(&locked.standard_input(), locked.nondeterminism())
                    .is_err(),
                "legacy-tagged locked coin must be rejected (tasm), tag {tag}"
            );

            // (b) released: release in the past -> accepted by rust AND tasm.
            let released = make_witness_with_tagged_coins(
                tag,
                Timestamp::now() - Timestamp::days(1),
                Timestamp::now(),
            );
            assert!(
                TimeLockV2
                    .run_rust(&released.standard_input(), released.nondeterminism())
                    .is_ok(),
                "legacy-tagged released coin must be accepted (rust), tag {tag}"
            );
            assert!(
                TimeLockV2
                    .run_tasm(&released.standard_input(), released.nondeterminism())
                    .is_ok(),
                "legacy-tagged released coin must be accepted (tasm), tag {tag}"
            );
        }
    }
    ...
    #[test]
    fn legacy_tagged_timelock_targets_apply_branch_correct_extension() {
        // The +1 year extension is granted only in the EXTENSION branch (legacy
        // + current TimeLock); legacy TimeLockV2 coins sit in the PLAIN branch and
        // never get the extension, even when the UTXO is a +1 year target. The
        // existing `v2_target_*` tests only cover the *current* TimeLock hash;
        // here we pin the behaviour for the legacy (pre-UpgradeVM) hashes.
        let release_date = Timestamp::now();

        // (1) legacy TimeLock as a target, tx within the +1 year window: the
        // extension pushes the effective release past `now` -> rejected.
        let within = release_date + Timestamp::years(1) - Timestamp::millis(1);
        let w = make_witness_with_tagged_coins(
            TimeLock::legacy_type_script_hash(),
            release_date,
            within,
        );
        let prog = TimeLockV2WithTargets(targets_covering(&w));
        assert!(
            prog.run_rust(&w.standard_input(), w.nondeterminism()).is_err(),
            "legacy TimeLock target within +1y must be rejected (rust)"
        );
        assert!(
            prog.run_tasm(&w.standard_input(), w.nondeterminism()).is_err(),
            "legacy TimeLock target within +1y must be rejected (tasm)"
        );

        // (2) legacy TimeLock as a target, tx past the +1 year window -> accepted.
        let past = release_date + Timestamp::years(1) + Timestamp::days(1);
        let w = make_witness_with_tagged_coins(
            TimeLock::legacy_type_script_hash(),
            release_date,
            past,
        ...
        let prog = TimeLockV2WithTargets(targets_covering(&w));
        assert!(
            prog.run_rust(&w.standard_input(), w.nondeterminism()).is_ok(),
            "legacy TimeLock target past +1y must be accepted (rust)"
        );
        assert!(
            prog.run_tasm(&w.standard_input(), w.nondeterminism()).is_ok(),
            "legacy TimeLock target past +1y must be accepted (tasm)"
        );

        // (3) legacy TimeLockV2 as a target -> PLAIN branch, NO extension: a coin
        // whose release date is already in the past is spendable now despite being
        // a target (the extension would otherwise keep it locked for a year).
        let past_release = Timestamp::now() - Timestamp::days(1);
        let w = make_witness_with_tagged_coins(
            TimeLockV2::legacy_type_script_hash(),
            past_release,
            Timestamp::now(),
        ...
        assert!(
            prog.run_rust(&w.standard_input(), w.nondeterminism()).is_ok(),
            "legacy TimeLockV2 target (plain branch) released must be accepted (rust)"
        );
        assert!(
            prog.run_tasm(&w.standard_input(), w.nondeterminism()).is_ok(),
            "legacy TimeLockV2 target (plain branch) released must be accepted (tasm)"
        );
    }
