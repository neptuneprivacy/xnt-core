...
use crate::prelude::twenty_first::math::traits::{
    CyclicGroupGenerator, ModPowU64, PrimitiveRootOfUnity,
};
use crate::prelude::twenty_first::prelude::{Inverse, ModPowU32};
use anyhow::Result;
use bip39::Mnemonic;
...
pub struct SecretKeyMaterial(pub(crate) BField32Bytes);

impl Zeroize for SecretKeyMaterial {
    fn zeroize(&mut self) {
        self.0 = BField32Bytes::new([BFieldElement::zero(); 4]);
    }
}
...
#[cfg_attr(any(test, feature = "arbitrary-impls"), derive(arbitrary::Arbitrary))]
pub(crate) struct BField32Bytes(pub(crate) [BFieldElement; 4]);
