...
impl RustyWalletDatabase {
    ...
    async fn try_connect_internal(
        ...
        if is_new_db {
            ...
            match schema_version.cmp(&WALLET_DB_SCHEMA_VERSION) {
                ...
                // database is too new, probably from a newer xnt-core binary.
                Ordering::Greater => {
    ...
    pub fn get_dctidh_key_counter(&self) -> u64 {
        self.tables.dctidh_key_counter.get()
    }

    /// set wallet derivation counter for CTIDH keys
    pub async fn set_dctidh_key_counter(&mut self, counter: u64) {
        self.tables.dctidh_key_counter.set(counter).await;
    }
...
pub enum WalletDbConnectError {
    ...
    #[error("Wallet database schema version is higher than expected.  It appears to come from a newer release of xnt-core.  expected schema version: {}, found: {}", expected, found)]
