...
/// The `xnt-core` command-line program starts a Neptune node.
...
pub struct Args {
    ...
    /// spawned Triton VM instances, not by xnt-core.
    ///
    ...
    #[clap(long, default_value = "9898", value_name = "PORT")]
    pub peer_port: u16,
    ...
    /// IPs of nodes to connect to, e.g.: --peer 8.8.8.8:9898 --peer 8.8.4.4:1337.
    #[structopt(long = "peer")]
    ...
    /// non-mining operations may panic and halt xnt-core
    ///
    ...
    ///
    /// Examples:
    ///  - `xnt-core --scan-blocks ..` (scan all blocks; this is the
    ///    default)
    ///  - `xnt-core --scan-blocks ..1337` (everything up to 1337)
    ///  - `xnt-core --scan-blocks 1337..` (1337 and everything after)
    ///  - `xnt-core --scan-blocks 13..=37` (13, 37, and everything in
    ///    between)
    ///  - `xnt-core --scan-blocks 13:37` (python index ranges also work)
    //
    ...
    \n\
    Examples: \n\
     - `xnt-core --scan-blocks ..` (scan all blocks; this is the default)\n\
     - `xnt-core --scan-blocks ..1337` (everything up to 1337)\n\
     - `xnt-core --scan-blocks 1337..` (1337 and everything after)\n\
     - `xnt-core --scan-blocks 13..=37` (13, 37, and everything in\n\
       between)\n\
    ...
    ///
    /// Example: `xnt-core --scan-keys 42`
    #[clap(long)]
    pub(crate) scan_keys: Option<usize>,

    /// Enable JSON/HTTP RPC.
    /// You can optionally specify an address and port (default: 127.0.0.1:9897).
    /// If not given, RPC is disabled.
    #[clap(
        long,
        default_missing_value = "127.0.0.1:9897",
        num_args = 0..=1,
        value_name = "ADDR"
    )]
...
mod tests {
    ...
    fn default_args_test() {
        ...
        assert_eq!(9898, default_args.peer_port);
        assert_eq!(9899, default_args.rpc_port);
        assert_eq!(
