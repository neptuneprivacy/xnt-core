...
/// enumerates xnt-core RPC authentication token types
///
...
///
/// a cookie file is created each time xnt-core is started.
///
...
///   1. proves to xnt-core that the client is on the same device and
///      has read access for files written by xnt-core.
...
/// provides a hint xnt-core client can use to automate authentication
