...
//! To do so, one should add neptune-privacy as a dependency and
...
//!
//! // query xnt-core server how to find the cookie file
...
pub trait RPC {
    ...
    /// xnt-core instance as well as the [Network].
    ///
    /// ```no_run
    /// # use anyhow::Result;
    /// # use neptune_privacy::application::rpc::server::RPCClient;
    ...
    /// Return the network this xnt-core instance is running
    ///
    /// ```no_run
    /// # use anyhow::Result;
    /// # use neptune_privacy::application::rpc::server::RPCClient;
    ...
    /// # let client = RPCClient::new(client::Config::default(), transport).spawn();
    /// #
    /// // query xnt-core server the network it is running on.
    ...
    /// # let token : auth::Token = auth::Cookie::try_load(&cookie_hint.data_directory).await?.into();
    /// #
    /// // query xnt-core server to get local socket used for incoming peer-connections.
    ...
    /// # let token : auth::Token = auth::Cookie::try_load(&cookie_hint.data_directory).await?.into();
    /// #
    /// // query xnt-core server to get own instance ID.
    ...
    /// # let token : auth::Token = auth::Cookie::try_load(&cookie_hint.data_directory).await?.into();
    /// #
    /// // query xnt-core server to get the block height.
    /// let block_height = client.block_height(context::current(), token).await??;
    /// # Ok(())
    /// # }
    /// ```
    ...
    /// # let token : auth::Token = auth::Cookie::try_load(&cookie_hint.data_directory).await?.into();
    /// #
    /// // query xnt-core server to get the blocks since wallet balance changed.
    ...
    /// # let token : auth::Token = auth::Cookie::try_load(&cookie_hint.data_directory).await?.into();
    /// #
    /// // query xnt-core server to get the info about the peers we are connected to
    ...
    /// # let token : auth::Token = auth::Cookie::try_load(&cookie_hint.data_directory).await?.into();
    /// #
    /// // query xnt-core server to get the info about the peers that are negatively sanctioned
    ...
    /// let n : usize = 10;
    ///
    /// // query xnt-core server to get the digests of the n latest blocks
    ...
    ///
    /// // query xnt-core server to get block info
    ...
    ///
    /// // query xnt-core server to block digests by height
    ...
    ///
    /// // query xnt-core server to get block digest
    ...
    ///
    /// // query xnt-core server to get utxo digest
    ...
    /// let block_selector : BlockSelector = BlockSelector::Special(BlockSelectorLiteral::Genesis);
    ///
    /// // query xnt-core server to get block header
    ...
    /// # let token : auth::Token = auth::Cookie::try_load(&cookie_hint.data_directory).await?.into();
    /// #
    /// // query xnt-core server to get sum of confirmed unspent UTXO
    ...
    /// # let token : auth::Token = auth::Cookie::try_load(&cookie_hint.data_directory).await?.into();
    /// #
    /// // query xnt-core server to get sum of unconfirmed unspent UTXOs
    ...
    /// # let token : auth::Token = auth::Cookie::try_load(&cookie_hint.data_directory).await?.into();
    /// #
    /// // query xnt-core server to get history of transactions, a vec containing digest, block height, timestamp and neptune coins tuples.
    ...
    /// # let token : auth::Token = auth::Cookie::try_load(&cookie_hint.data_directory).await?.into();
    /// #
    /// // query xnt-core server to get the funds in the wallet
    ...
    /// # let token : auth::Token = auth::Cookie::try_load(&cookie_hint.data_directory).await?.into();
    /// #
    /// // query xnt-core server to get the number of expected utxos including already received ones.
    ...
    ///
    /// // query xnt-core server to get a receiving address
    ...
    /// # let token : auth::Token = auth::Cookie::try_load(&cookie_hint.data_directory).await?.into();
    /// #
    /// // query xnt-core server to get all known keys for every [KeyType]
    ...
    ///
    /// // query xnt-core server to get all known keys by [KeyType]
    ...
    /// # let token : auth::Token = auth::Cookie::try_load(&cookie_hint.data_directory).await?.into();
    /// #
    /// // query xnt-core server to get the number of transactions in the mempool
    ...
    /// let number : usize = 8;
    ///
    /// // query xnt-core server to get the info of transactions in the mempool
    ...
    /// # let token : auth::Token = auth::Cookie::try_load(&cookie_hint.data_directory).await?.into();
    /// #
    /// // query xnt-core server to get the info used on dashboard overview tab
    ...
    ///
    /// // query xnt-core server to check if the supplied address is valid
    ...
    ///
    /// // query xnt-core server to determine if the amount is valid
    ...
    ///
    /// // query xnt-core server to determine if the amount is less than or equal to the balance
    ...
    /// # let token : auth::Token = auth::Cookie::try_load(&cookie_hint.data_directory).await?.into();
    /// #
    /// // query xnt-core server to get the list of owned and unspent coins
    ...
    /// # let token : auth::Token = auth::Cookie::try_load(&cookie_hint.data_directory).await?.into();
    /// #
    /// // query xnt-core server to get the list of UTXOs
    ...
    /// # let token : auth::Token = auth::Cookie::try_load(&cookie_hint.data_directory).await?.into();
    /// #
    /// // query xnt-core server instance to get its CPU temperature
    ...
    ///
    /// // query xnt-core server to get difficulties of a range of blocks
    ...
    /// # let token : auth::Token = auth::Cookie::try_load(&cookie_hint.data_directory).await?.into();
    /// #
    /// // xnt-core server clears standing for all peers that are connected or not
    ...
    /// # use neptune_privacy::application::rpc::server::RPCClient;
    /// # use neptune_privacy::application::rpc::auth;
    /// # use neptune_privacy::api::export::ChangePolicy;
    /// # use neptune_privacy::api::export::OutputFormat;
    /// # use tarpc::tokio_serde::formats::Json;
    /// # use tarpc::serde_transport::tcp;
    /// # use tarpc::client;
    /// # use tarpc::context;
    ...
    /// #
    /// // xnt-core server sends token to a single recipient
...
pub(crate) struct NeptuneRPCServer {
    ...
    // copy of DataDirectory for this xnt-core instance.
...
impl NeptuneRPCServer {
    ...
    /// get the data_directory for this xnt-core instance
...
impl RPC for NeptuneRPCServer {
    ...
    async fn spendable_inputs(
        ...
        Ok(self
            ...
            .api()
            .tx_initiator()
            .spendable_inputs(Timestamp::now(), exclude_recent_blocks)
            .await)
    }
...
mod tests {
    ...
    async fn restore_membership_proof_privacy_preserving_devnet_wallet() {
        ...
        let utxo = rpc_server
            ...
            .lock_guard()
            .await
            .wallet_spendable_inputs(Timestamp::now(), 0)
            .await
            .into_iter()
            .collect_vec()[0]
            .clone();
    ...
    mod send_tests {
        ...
        mod worker {
            use super::*;
            use crate::state::wallet::address::dctidh_address::{dCTIDHSpendingKey, dCTIDHSubAddress};
            use crate::state::wallet::address::generation_address::GenerationReceivingAddress;
            ...
            pub(super) async fn send_to_many(recipient_key_type: KeyType) -> Result<()> {
                ...
                let external_receiving_address: ReceivingAddress = match recipient_key_type {
                    ...
                    KeyType::dCTIDH => dCTIDHSpendingKey::keygen().to_address().into(),
                    KeyType::dCTIDHSubAddr => {
                        let addr = dCTIDHSpendingKey::keygen().to_address();
                        let mut payment_id: BFieldElement = rng.random();
                        if payment_id.is_zero() {
                            payment_id = BFieldElement::new(1);
                        }
                        dCTIDHSubAddress::new(addr, payment_id).unwrap().into()
                    }
                };
