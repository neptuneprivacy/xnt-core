...
use num_traits::Zero;
use neptune_privacy::state::wallet::address::ctidh_address::{
    CtidhReceivingAddress, CtidhSpendingKey, CTIDH_ADDRESS_MAX_BECH32M_LEN,
};
...
#[test]
fn ctidh_address_bech32m_roundtrip_and_length() {
    ...
    let decoded_ctidh = CtidhReceivingAddress::from_bech32m(&long, NETWORK).expect("from_bech32m");
    assert_eq!(addr, decoded_ctidh);
}

/// ReceivingAddress parses CTIDH from bech32m (prefix xntct).
#[test]
fn receiving_address_from_bech32m_parses_ctidh() {
    let key = CtidhSpendingKey::keygen();
    let addr = key.to_address();
    ...
    match &parsed {
        ReceivingAddress::Ctidh(_) => {}
        _ => panic!("expected ReceivingAddress::Ctidh, got {:?}", parsed),
    }
...
async fn alice_sends_to_bob_ctidh_address() -> anyhow::Result<()> {
    ...
    let bob_ctidh_address = bob
        .gsl
        .api_mut()
        .wallet_mut()
        .next_receiving_address(KeyType::Ctidh)
        .await?;
    println!("bob_ctidh_address: {}", bob_ctidh_address.to_bech32m(NETWORK).unwrap());
    ...
    let tx_artifacts = alice
        ...
        .tx_sender_mut()
        .send(
            vec![(bob_ctidh_address, payment_amount)],
            Default::default(),
            fee_amount,
            Timestamp::now(),
            0,
...
#[tokio::test(flavor = "multi_thread")]
async fn alice_sends_to_bob_ctidh_bech32m_address() -> anyhow::Result<()> {
    logging::tracing_logger();
    let timeout_secs = 5u16;
    ...
    let bech32_addr = bob_ctidh_address.to_bech32m(NETWORK)?;
    let bob_ctidh_address_from_bech32 =
    ...
    let tx_artifacts = alice
        ...
        .tx_sender_mut()
        .send(
            vec![(bob_ctidh_address_from_bech32, payment_amount)],
            Default::default(),
            fee_amount,
            Timestamp::now(),
            0,
